...
export default function AdminOrders() {
  ...
  // 手機端卡片式訂單項目
  const OrderCard = ({ order, showReviewActions }: { order: any; showReviewActions: boolean }) => (
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex gap-1.5">
          ...
          {order.paymentStatus === "pending" && order.paymentMethod !== "bank_transfer" && (
            <Button size="sm" variant="outline" className="h-7 text-xs px-2" onClick={() => { if (confirm("確認此訂單已付款？")) confirmPayment.mutate({ orderNo: order.orderNo }); }}>
              <CheckCircle className="h-3 w-3 mr-1" />確認付款
            </Button>
          )}
        </div>
      </div>
  ...
  // 桌面端表格
  const renderDesktopTable = (orders: any[] | undefined, loading: boolean, showReviewActions: boolean) => (
  ...
  // 手機端卡片列表
  const renderMobileCards = (orders: any[] | undefined, loading: boolean, showReviewActions: boolean) => (
    <div className="md:hidden">
      {loading ? (
        ...
        orders.map((order) => <OrderCard key={order.id} order={order} showReviewActions={showReviewActions} />)
      )}
    </div>
  );
        ...
        <TabsContent value="all" className="space-y-4">
          ...
          <Card className="bg-card border-border">
            <CardContent className="p-0">
              {renderDesktopTable(data?.items, isLoading, true)}
              {renderMobileCards(data?.items, isLoading, true)}
            </CardContent>
          </Card>
          {data && data.total > 20 && (
        ...
        <TabsContent value="pending_review" className="space-y-4">
          {pendingReviews && pendingReviews.total > 0 ? (
            <>
              ...
              <Card className="bg-card border-border">
                <CardContent className="p-0">
                  {renderDesktopTable(pendingReviews.items, false, true)}
                  {renderMobileCards(pendingReviews.items, false, true)}
                </CardContent>
              </Card>
            </>
          ) : (
